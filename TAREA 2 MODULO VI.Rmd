...
Podemos obeservar que las mejores ligas que mantienen la distancia son la promedio, la simple y la centroid
...
plot(hc_centroid)
```
Podemos observar que con los dendogramas con las ligas single, average y centroid no podemos afirmar cuántos grupos podemos tener de toda nuestra base de datos haciendo algún corte.
...
Con los dendogramas anteriores no podemos hacer una afirmación de cuántos cluster podemos tener en nuestra base de datos.

Grafiquemos el dendograma con la liga Ward la cual es mejor y más visual considerando 3 clusters.
```{r}
...
Otra alternativa es hacer una receta que incluye todas las variables de nuestro dataset, excluyendo la variable "Potability" del procesamiento.
```{r}
...
Ahora realizamos una gráfica llamada gráfica de "Codo" para visualizar la proporción WSS/TSS o sse_ratio que es una medida de la proporción de la varianza total para buscar un clúster óptimo. 
```{r}
res1_metrics %>%
...
```
Observando la gráfica, el “codo” parece estar cerca del cluster número 3 o 4, donde hay un cambio más notable en la pendiente. Por lo que el número óptimo de clusters probablemente esté entre 3 y 4. Más de 5 clústers no suele ser óptimo ya que a partir del cluster 5 las reducciones son cada vez menores, es decir, la curva se aplana), lo que sugiere que agregar más clusters ya no mejora significativamente.
...
### IDEAL OPTICS
El algoritmo OPTICS (siglas de Ordering Points To Identify the Clustering Structure) es un algoritmo de clustering jerárquico basado en densidad, diseñado para identificar estructuras de clusters de distintas densidades.
...
Aplicamos el modelo.
```{r}
datosOptics <- optics(datos_optics, minPts = 10)
plot(datosOptics)
```
Desde esta visualización notamos no hay clusters bien definidos a simple vista. Los valles, que representarían clusters densos, no son evidentes aquí.
Por lo tanto, este algoritmo para nuestro dataset no es viable.

### ModelBasedClusteringWDBC
Comparación de modelos de clustering usando el algoritmo de model-based clustering
...
En mclust, mayor BIC = mejor modelo. En este caso el valor más alto es 83,743, correspondiente a k = 1 cluster lo que sugiere que, según el criterio BIC, no hay evidencia suficiente para justificar múltiples grupos.
**Nota:** BIC es una métrica estadística utilizada para comparar modelos y seleccionar el que mejor se ajusta a los datos.
...
plot(BIC)
```
En esta gráfica vemos que en este caso es el modelo VVI.

Lo comprobamos haciendo un summary.
```{r}
summary(BIC)
```
...
**Conclusion:** Hay una superposición significativa entre clusters en muchas combinaciones de variables, lo que indica que no todos los atributos separan bien los grupos. Por lo que con 3 clusters tampoco tenemos una separación óptima. 
